...
public class PlayerController : MonoBehaviour
{
    ...
    public GameObject colliderWhenFrized;
...
//    private float deltaFromLastColl = 0f;
    ...
    void Start()
    {
        colliderWhenFrized.SetActive(false);
    ...
    void Update()
    {
        ...
        if (IsHoldingBall)
        {
            colliderWhenFrized.SetActive(true);
        ...
        if (isGrounded)//velocity_Y == 0 || (velocity_Y < 0 && ac.GetInteger("state") != JUMP)) // If the cat is standing still or falling (without jumping)
        {
            audioSource.Pause();
    ...
    public void Catch(Collider2D other)
    {
        if (other.GetComponent<BallController>().isHeldByCat)
        {
            return;
        }
